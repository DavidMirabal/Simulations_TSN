...
class Figura:
    ...
    def xlabel(self, index_ax=-1, s=r""):
        self.ax[index_ax].set_xlabel(r"$" + s + r"$", color=self.c_text, fontsize=self.fontsize)

    def ylabel(self, index_ax=-1, s=r""):
        self.ax[index_ax].set_ylabel(r"$" + s + r"$", color=self.c_text, fontsize=self.fontsize)
